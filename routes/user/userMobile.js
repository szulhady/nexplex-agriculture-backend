...
router.post('/register', async(req,res) =>{
  ...
  if( error.length > 0) {
    ...
    const queryFindByEmail = `SELECT * FROM users_nexplex_agriculture_mobile WHERE email="${email}"`;
           ...
           return res.send(error)
            ...
            bcrypt.genSalt(10, (err,salt) =>{
              bcrypt.hash(password,salt, (err,hash) =>{
                ...
                const queryInsertNewUser = `INSERT INTO users_nexplex_agriculture_mobile SET ?`;
                connection.query(
                  queryInsertNewUser,
                  newUser,
                  (err,results) =>{
